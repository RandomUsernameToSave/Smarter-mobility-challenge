...
def format_submission2(station_prediction: pd.DataFrame, area_prediction: pd.DataFrame, global_prediction: pd.DataFrame, test_station: pd.DataFrame):
    """
    Transform the predictions into a suitable format before submission

    inputs:
        station_prediction (pd.DataFrame):
            DataFrame containing the results of the prediction at the station level, must contain the following columns: date, Station, Available, Charging, Passive, Other
        area_prediction (pd.DataFrame):
            DataFrame containing the results of the prediction at the area level, must contain the following columns: date, area, Available, Charging, Passive, Other
        global_prediction (pd.DataFrame):
            DataFrame containing the results of the prediction at the global level, must contain the following columns: date, Available, Charging, Passive, Other
        test_station (pd.DataFrame): DataFrame containing the test.csv data.
    """
    ...
    area_prediction = pd.merge(test_area, area_prediction, on=[
                               'date','area'])
    global_prediction = pd.merge(
        test_global, global_prediction, on=['date'])
